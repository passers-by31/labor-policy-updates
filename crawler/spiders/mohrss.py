...
class MohrssSpider(Spider):
    ...
    def parse_list(self, html: str, list_config: dict[str, Any]) -> list[dict[str, str]]:
        ...
        selector = list_config.get("list_selector", "ul.list-list li a")
    ...
    def parse_detail(self, html: str, base_url: str) -> dict[str, Any] | None:
        ...
        # 标题
        title = ""
        for sel in ["div.content-title h1", "div.article-title h1", "h1"]:
        ...
        # 发布日期
        publish_date = ""
        for sel in ["div.content-info span.date", "div.article-info span.date", "div.info span"]:
            el = soup.select_one(sel)
            if el:
        ...
        # 文号
        doc_number = ""
        for sel in ["div.content-info", "div.article-info"]:
            el = soup.select_one(sel)
            if el:
        ...
        raw = f"{base_url}_{publish_date or datetime.date.today().isoformat()}"
